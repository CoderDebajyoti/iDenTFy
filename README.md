...
| **Backend** | Web Service (Python 3.12) | `backend` | `pip install -r requirements.txt` | `uvicorn app.main:app --host 0.0.0.0 --port $PORT` | `DATABASE_URL` (From Postgres)<br>`FRONTEND_URL` (e.g. `https://identfy-frontend.onrender.com`)<br>`DATABASE_MATCHING_ENABLED`=`false`<br>`BYPASS_DATABASE_MATCHING`=`true`<br>`FACE_SIMILARITY_THRESHOLD`=`0.68`<br>`ENV`=`production`<br>`DEBUG`=`false` |
